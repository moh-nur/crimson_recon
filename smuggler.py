...
def generateBaselineAttackMessage( base_message, method, attack_datas ):
    try:
        f = getattr( am, method )
    except Exception as e:
        return ''
...
def testURL( url ):
    ...
    for method in t_methods:
        for attack_datas in t_attacks_datas:
            ...
            attack_baseline_message = generateBaselineAttackMessage( base_message, method, attack_datas )
            ...
            r = doRequest(url, attack_baseline_message)
            if r.status_code < 0:
                t_exceptions[url] = t_exceptions[url] + 1
            else:
                save_code = r.status_code
                r = doRequest( url, msg )
                if r.status_code < 0:
                    t_exceptions[url] = t_exceptions[url] + 1
                else:
                    if r.time > 5000 and r.status_code != save_code and r.content != 'upstream request timeout':
